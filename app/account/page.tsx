...
export default function AccountSettings() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (userDataResult) {
          ...
          setSelectedPlanId(userDataResult.planId || '');
        }
  ...
  const handlePlanChange = async () => {
    ...
    try {
      ...
      await updateCurrentUserPlan(selectedPlanId);
      
      // Update local state
      const selectedPlan = plans.find(plan => plan.id === selectedPlanId);
      if (selectedPlan && userData) {
        setUserData({ ...userData, plan: selectedPlan.name, planId: selectedPlanId });
      }
  ...
  const handleCancelPlanChange = () => {
    setSelectedPlanId(userData?.planId || '');
    setShowPlanSelection(false);
    setError('');
  };
                ...
                <div className="flex items-center space-x-4">
                  <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center">
                    <span className="text-xl font-medium text-green-600">
                      {userData.displayName ? userData.displayName.substring(0, 2).toUpperCase() : 'U'}
                    </span>
                  </div>
                  <div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <p className="text-lg font-semibold text-green-600">${currentPlan.price}/{currentPlan.billingCycle}</p>
                    </div>
                    <button
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 font-medium">{currentPlan.limits.bots}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Messages/Day:</span>
                      <span className="ml-2 font-medium">{currentPlan.limits.messagesPerDay}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Storage:</span>
                      <span className="ml-2 font-medium">{currentPlan.limits.storage}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Team Members:</span>
                      <span className="ml-2 font-medium">{currentPlan.limits.teamMembers}</span>
                    </div>
                  </div>
                </div>
              ) : (
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mt-6">
                  ...
                  {plans.map((plan) => (
                    ...
                    >
                      <div className="text-center">
                        ...
                        <p className="text-sm text-gray-600">/{plan.billingCycle}</p>
                ...
                <div className="flex space-x-4">
                  <button
                    ...
                    className="flex-1 bg-green-500 hover:bg-green-600 disabled:bg-gray-400 text-white px-6 py-2 rounded-lg font-medium transition-colors disabled:cursor-not-allowed"
                  >
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900">
                    {userData.joined ? new Date(userData.joined.toDate()).toLocaleDateString() : 'N/A'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Last Active</span>
                  <span className="text-sm font-medium text-gray-900">
                    {userData.lastActive ? new Date(userData.lastActive.toDate()).toLocaleDateString() : 'N/A'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900">{userData.bots || 0}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Revenue</span>
                  <span className="text-sm font-medium text-gray-900">${userData.revenue || 0}</span>
                </div>
              </div>
            </div>
