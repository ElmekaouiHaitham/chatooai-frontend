          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{plans.reduce((sum, plan) => sum + (plan.users || 0), 0)}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">${plans.reduce((sum, plan) => sum + (plan.revenue || 0), 0).toLocaleString()}</p>
              </div>
                ...
                <tbody className="divide-y divide-gray-200">
                  {plans.map((plan) => (
                    <tr key={plan.id} className="hover:bg-gray-50">
                      ...
                      <td className="py-4 px-6 text-sm text-gray-900">{plan.users || 0}</td>
                      <td className="py-4 px-6 text-sm text-gray-900">${plan.revenue || 0}</td>
                      <td className="py-4 px-6">
          ...
          <div className="mt-8 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  {plans.map((plan) => (
                    <div key={plan.id} className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <span className={`w-3 h-3 rounded-full ${getPlanColor(plan.name).replace('bg-', '').split(' ')[0]}`}></span>
                        <span className="text-sm font-medium">{plan.name}</span>
                      </div>
                      <span className="text-sm text-gray-600">{plan.users || 0} users</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {plans.filter(p => (p.revenue || 0) > 0).map((plan) => (
                    <div key={plan.id} className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <span className={`w-3 h-3 rounded-full ${getPlanColor(plan.name).replace('bg-', '').split(' ')[0]}`}></span>
                        <span className="text-sm font-medium">{plan.name}</span>
                      </div>
                      <span className="text-sm text-gray-600">${plan.revenue || 0}</span>
                    </div>
                  ))}
                </div>
              </div>
