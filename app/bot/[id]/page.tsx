...
//   const [whatsappStatus, setWhatsappStatus] = useState<'disconnected' | 'connecting' | 'connected' | 'error'>('disconnected');
...
//       const result = await whatsappService.connectBot(bot.id);
...
//     try {
//       await whatsappService.disconnectBot(bot.id);
...
//                     <p className="text-gray-600">Click "Connect to WhatsApp" to start your bot.</p>
//                   </div>
//                 )}
//               </div>
//             </div>
...
//                   {Object.entries(bot.features).map(([key, value]) => (
...
export default function QRPage() {
  const params = useParams();
  const botId = params.id; // Get bot ID from URL
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/chatbot/${botId}`)
      .then(res => res.json())
      .then(data => setBotName(data.name));
  ...
  return (
    <div style={{ padding: 20 }}>
      <h1>Scan WhatsApp QR Code for {botName}</h1>
      {qr ? <img src={qr} alt={`QR Code for ${botName}`} /> : <p>Waiting for QR...</p>}
    </div>
  );
}
