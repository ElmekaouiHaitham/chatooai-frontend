...
export const signInWithGoogle = async (planId?: string) => {
  try {
    ...
    await createUserDocument(result.user, result.user.displayName || undefined, planId);
...
export const signUpWithEmail = async (email: string, password: string, displayName?: string, planId?: string) => {
  try {
...
export const createPlan = async (planData: Omit<PlanData, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
  try {
    const plansRef = collection(db, 'plans');
...
export const updatePlan = async (planId: string, planData: Partial<PlanData>): Promise<void> => {
  try {
    const planRef = doc(db, 'plans', planId);
...
export const ensureBotsCollection = async (): Promise<void> => {
  try {
    ...
    console.log('Bots collection may not exist yet, this is normal for new users');
  }
};
...
export const getUserBots = async (uid: string): Promise<BotData[]> => {
  try {
    ...
    const botsRef = collection(db, 'bots');
    const q = query(botsRef, where('uid', '==', uid), orderBy('createdAt', 'desc'));
    const querySnapshot = await getDocs(q);
    ...
    if (error instanceof Error && (
      ...
      console.log('Bots collection issue, returning empty array:', error.message);
      return [];
    }
...
export const updateBot = async (botId: string, updates: Partial<BotData>): Promise<void> => {
  try {
    const botRef = doc(db, 'bots', botId);
...
export const updateBotWhatsAppStatus = async (botId: string, status: BotData['whatsapp']['status'], qrCode?: string): Promise<void> => {
  try {
    const botRef = doc(db, 'bots', botId);
    const updates: any = {
...
export const updateBotStats = async (botId: string, stats: Partial<BotData['stats']>): Promise<void> => {
  try {
    const botRef = doc(db, 'bots', botId);
    await updateDoc(botRef, {
      ...Object.fromEntries(
